...
public class FlavorIntegrationTest extends AbstractIntegrationTest {
    @Autowired
    ...
    private String baseUrl = "/v1/flavors";
    ...
    public void should_have_throw_EntityNotFoundException_when_id_not_found() throws Exception {
        ...
        this.mockMvc.perform(MockMvcRequestBuilders.get(url))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("error", is("Flavor by id: 100 Not found")))
                .andDo(documentationHandler.document(PayloadDocumentation.responseFields(FieldDescriptToErrors.getDefaultFieldDescript())));
    }

    @Test
    public void should_return_specific_flavor_when_send_id() throws Exception {
        var url = baseUrl + "/3";
        var flavor = flavorRepository.findById(3).orElseThrow(EntityNotFoundException::new);

        mockMvc.perform(MockMvcRequestBuilders.get(url))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("id", is(flavor.getId())))
                .andExpect(MockMvcResultMatchers.jsonPath("name", is(flavor.getName())))
                .andExpect(MockMvcResultMatchers.jsonPath("addons.id", is(flavor.getAddons().getId())))
                .andExpect(MockMvcResultMatchers.jsonPath("addons.name", is(flavor.getAddons().getName())))
                .andExpect(MockMvcResultMatchers.jsonPath("addons.time", is(5)))
                .andDo(documentationHandler.document(
    ...
    public void should_have_update_flavor_successfully() throws Exception {
        var id = 1;
        var url = baseUrl + "/" + id;
        var flavor = flavorRepository.findById(id).orElseThrow(EntityNotFoundException::new);
